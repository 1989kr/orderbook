...
class Order(object):
    ...
    def print_trade_result(self, other_order_id):
        if self.trade_size > 0:
            if self.is_bid:
                print({'buyOrderId': self.id, 'sellOrderId': other_order_id, 'price': self.price, 'quantity': self.trade_size})
                
            else:
                print({'buyOrderId': other_order_id, 'sellOrderId': self.id, 'price': self.price, 'quantity': self.trade_size})
            self.trade_size = 0
